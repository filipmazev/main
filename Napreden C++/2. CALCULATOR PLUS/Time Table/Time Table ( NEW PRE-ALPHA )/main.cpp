...
Main:
    {
        ...
        third, dayEntered, dayEnteredOriginal, DayInMonthCheck, chsInYear, chsInYear2, yearMain, monthMain, yearCompare, monthCompare, cmpr = 4, LeapYearCode = 0, MonthCode = 0, YearCode=0, CenturyCode=0, dayDiferanceOriginal, chsHere5;
            long_double_t sum2=0.00, sum=0.00, num[1000], numAR[] = { 31557600, 31622400, 86400, 3600, 60, 1, 1000, 1000000, 1000000000 }, dev[9], sumN=0, caseNum = 0, sumMLDirect, sumSDirect, sumMDirect, sumHDirect, sumDDirect, sumYDirect, sumLYDirect, sumMCRDirect, sumNNSDirect, sumOriginal=0.00, totalDiferanceLeapYear,
        ...
        cout<<" (1) - TIME CONVERTOR  - (1) "<<endl;
        cout<<" (2) - TIME DIFFERENCE - (2) "<<endl;
        cout<<" (3) - EXTRA OPTIONS   - (3) "<<endl;
        cout<<endl<<" Enter:"; cin>>chsMain;
        while(cin.fail()){error(); cin>>chsMain;}
        ...
        if(chsMain == 1)
        {
            ...
            cout<<" TIME TABLE "<<checkerGet2<<endl; cout<<endl;
                ...
                sumOriginal = sum; sumN = sum;

                system ("cls"); cout<<" TIME TABLE "<<checkerGet2<<endl; cout<<endl;

            if (chs == 1) {
                for ( int i=0; i<9; i++ ) { dev[i] = numAR[i] / 1; }
                    sumN /= dev[2]; c6 = "SECOND"; c62 = "seconds"; caseNum = 1; }
                if (chs == 2) {
                    for ( int i=0; i<9; i++ ) { if ( i < 5 ){ dev[i] = numAR[i] / 60; } else { dev[i] = numAR[i] * 60; } }
                        sumN /= dev[2]; c6 = "MINUTE"; c62 = "minute"; caseNum = 2; }
                    if (chs == 3){
                        for ( int i=0; i<9; i++ ){ if ( i < 4 ){ dev[i] = numAR[i] / 3600; } else { dev[i] = numAR[i] * 3600; } }
                            dev[4] = 60; sumN /= dev[2]; c6 = "HOUR"; c62 = "hour"; caseNum = 3;}
                        if (chs == 4){
                            dev[0] = 365.25; dev[1] = 366; dev[2] = 1; dev[3] = 24; dev[4] = 1440; dev[5] = 86400;
                                dev[6] = 86400000; dev[7] = 86400000000; dev[8] = 86400000000000; sumN /= dev[2]; c6 = "DAY"; c62 = "day"; caseNum = 4; }
                            if (chs == 5){
                                for ( int i=0; i<9; i++ ) { if ( i < 6 ){ dev[i] = numAR[i] * 1000; } else { dev[i] = numAR[i] / 1000; } }
                                    sumN /= dev[2]; c6 = "MILLISECOND"; c62 = "millisecond"; caseNum = 5; }
                                if (chs == 6){
                                    for ( int i=0; i<9; i++ ) { if ( i < 7 ){ dev[i] = numAR[i] * 1000000; } else dev[i] = numAR[i] / 1000000; }
                                        dev[6] /= 1000000; sumN /= dev[2]; c6 = "MICROSECOND"; c62 = "microsecond"; caseNum = 6; }
                                    if (chs == 7) {
                                        for ( int i=0; i<9; i++ ) { if ( i < 6 ){ dev[i] = numAR[i] * 1000000000; } else dev[i] = numAR[i] / 1000; }
                                            dev[8] = 1; dev[6] /= 1000000; sumN /= dev[2]; c6 = "NANOSECOND"; c62 = "nanosecond"; caseNum = 7; }

            if ( caseNum == 1 || caseNum == 2 || caseNum == 3 || caseNum == 4 || caseNum == 5 ){ sumMLDirect = sum * dev[6]; sumMCRDirect = sum * dev[7]; }
            else { sumMLDirect = sum / dev[6]; sumMCRDirect = sum / dev[7]; sumNNSDirect = sum / dev[8]; }
                if ( caseNum == 2 || caseNum == 3 || caseNum == 4 ) { sumSDirect = sum * dev[5]; } else { sumSDirect = sum / dev[5]; }
                    if ( caseNum == 3 || caseNum == 4 ) { sumMDirect = sum * dev[4]; } else { sumMDirect = sum / dev[4]; }
                        if ( caseNum == 4 ) { sumHDirect = sum * dev[3]; } else { sumHDirect = sum / dev[3]; }
                            if ( caseNum == 7 ) { sumNNSDirect = sum * dev[8]; } sumYDirect = sum / dev[0]; sumLYDirect = sum / dev[1]; sumDDirect = sum / dev[2];
